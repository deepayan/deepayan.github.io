...
opts_chunk$set(cache = FALSE, cache.path='~/knitr-cache/pooled-testing/', autodep = TRUE,
               comment = "", warning = TRUE, message = TRUE,
               fig.width = 12, fig.height = 6, dpi = 120, fig.path='figures/pooltest-')
library(lattice)
...
## sanity check
p <- 0.1; k <- 2; m <- 10000; q <- 1 - (1-p)^k
qhat <- rbinom(5000, m, 1-(1-p)^k) / m
phat <- 1 - (1-qhat)^(1/k)
sqrt(mean((phat - p)^2)) # empirical s.d.
sqrt(q * (1-p)^(2-k) / (m*k^2)) # delta method s.d.
```
