...
* An R function can quote an argument withut evaluating it

* This is done using the function `substitute()`

```{r}
getExpression <- function(x) return(substitute(x))
...
* A popular add-on package for data manipulation, called
  [dplyr](https://dplyr.tidyverse.org/), has a similar function

```{r}
dplyr::filter(g, b > a)
```

* [dplyr](https://dplyr.tidyverse.org/) has several other similar functions
...
* I will not exlore dplyr any further, but you should learn more about it online

* The `*apply()` type functions are generally useful, and we will discuss them soon
