...
* `lm()` is actually more numerically efficient (it does not actually compute `crossprod(X)`)
...
* The main reason is that R functions are now allowed to _modify_ their arguments
...
* Replacement functions are the exception to this rule
