...
* However, in this case average case is different from the worst case (and the best case)
--
...
* At the start of each iteration of the for loop in lines 3--7, the
...
$$
E(X) = \sub{\sum}{i=1}^{n-1} \sub{\sum}{j=i+1}^n E(\sub{X}{ij}) = \sub{\sum}{i=1}^{n-1}
\sub{\sum}{j=i+1}^n P(\sub{z}{i} \textsf{ is compared with } \sub{z}{j})
$$ 
